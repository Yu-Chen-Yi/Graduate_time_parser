...
class LabCensusSystem:
    ...
    def Search(self):
        ...
        while i < cnt:
            ...
            # 畢業生名字
            try:
                advisor_name = access.body.form.div.table.tbody.tr.td.table.find("th",text="指導教授:").find_next_sibling().get_text()
            except AttributeError:
                advisor_name = access.body.form.div.table.tbody.tr.td.table.find("th",text="指導教授:").find_next_sibling().get_text()
            # 畢業年 - 入學年
            ...
            if calculate == 0:
                print(enter_year+f"學年 "+student_name+f" "+part_time+f" : {calculate+1}年畢業 好孩子! 指導教授:"+advisor_name)
            if calculate == 1:
                print(enter_year+f"學年 "+student_name+f" "+part_time+f" : {calculate+1}年畢業 好孩子! 指導教授:"+advisor_name)
            if calculate == 2:
                print(enter_year+f"學年 "+student_name+f" "+part_time+f" : {calculate+1}年畢業 QQ      指導教授:"+advisor_name)
            if calculate == 3:
                print(enter_year+f"學年 "+student_name+f" "+part_time+f" : {calculate+1}年畢業 超QQ    指導教授:"+advisor_name)
            if calculate == 4:
                print(enter_year+f"學年 "+student_name+f" "+part_time+f" : {calculate+1}年畢業 巨QQ    指導教授:"+advisor_name)
            if calculate == 0:
                self.result[0] += 1
